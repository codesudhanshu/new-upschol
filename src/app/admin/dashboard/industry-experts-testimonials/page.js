...
import { Plus, Edit, Trash2, Eye, X, User, Briefcase, Building, Link2, MessageCircle } from 'lucide-react';
import { getAllIndustryExpertsTestimonials, deleteIndustryExpertTestimonials } from '@/app/api/admin/apiService';

const IndustryExpertsTestimonials = () => {
  const [experts, setExperts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedExpert, setSelectedExpert] = useState(null);

  // Fetch all industry experts testimonials
  const fetchExperts = async () => {
    try {
      setLoading(true);
      const response = await getAllIndustryExpertsTestimonials();
      if (response.status === true) {
        setExperts(response.result.experts || []);
      }
    } catch (error) {
      console.error('Error fetching industry experts testimonials:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (expertId) => {
    try {
      const response = await deleteIndustryExpertTestimonials(expertId);
      if (response.status === true) {
        setExperts(experts.filter(expert => expert._id !== expertId));
      }
    } catch (error) {
      console.error('Error deleting industry expert testimonial:', error);
    }
  };
    ...
    <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-8">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl shadow-lg p-6 mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Industry Experts Testimonials</h1>
            <p className="text-gray-600">All industry expert testimonials list of your website</p>
          </div>
          <Link href="/admin/dashboard/industry-experts-testimonials/create-industry-expert-testimonials">
            <button
            ...
            >
              <Plus className="w-5 h-5" />Add Industry Expert Testimonial
            </button>
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        {loading ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mt-4">Loading Industry Experts Testimonials...</p>
          </div>
        ) : experts.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-600">No industry expert testimonials found. Create your first one!</p>
          </div>
        ) : (
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 mb-2">Company: {expert.companyname}</p>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                      ...
                      {close => (
                        <div className="bg-white rounded-xl overflow-hidden">
                          ...
                          <div className="flex justify-between items-center p-4 border-b border-gray-200">
                            <h2 className="text-lg font-bold text-gray-900">Expert Testimonial Details</h2>
                            <button
                              onClick={close}
                            ...
                            <div className="grid grid-cols-1 gap-4">
                              ...
                              <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-600">{selectedExpert?.companyname}</p>
                              ...
                              <div className="bg-white border border-gray-100 rounded-lg p-4">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <InfoItem 
                                    icon={Building} 
                                    label="Company" 
                                    value={selectedExpert?.companyname} 
                                  />
                                  <InfoItem 
                                    icon={Link2} 
                                    label="LinkedIn URL" 
                                    value={selectedExpert?.linkedinurl} 
                                  />
                                  <div className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                                    <MessageCircle className="text-indigo-600 mt-0.5 flex-shrink-0" size={18} />
                                    <div className="min-w-0 flex-1">
                                      <p className="text-xs font-medium text-gray-500 mb-0.5">Testimonial</p>
                                      <p className="text-sm text-gray-900 whitespace-pre-wrap break-words">
                                        {selectedExpert?.testimonials || 'Not provided'}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                          ...
                          <div className="flex justify-end gap-3 p-4 border-t border-gray-200 bg-gray-50">
                            <Link href={`/admin/dashboard/industry-experts-testimonials/edit-industry-expert-testimonials/${selectedExpert?._id}`}>
                    ...
                    {/* Edit Button */}
                    <Link href={`/admin/dashboard/industry-experts-testimonials/edit-industry-expert-testimonials/${expert._id}`}>
                      <button
                        ...
                        <div className="bg-white rounded-xl overflow-hidden">
                          ...
                          <div className="p-6">
                            ...
                            <p className="text-gray-600 mb-6">
                              Are you sure you want to delete this industry expert testimonial? All associated data will be permanently removed.
                            </p>
                          </div>
...
export default IndustryExpertsTestimonials;