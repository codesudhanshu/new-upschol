...
import { createCompany } from '@/app/api/admin/apiService';
    ...
    try {
      ...
      if (result.status == true) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: result.error || 'Failed to add Company',
          confirmButtonColor: '#ef4444'
        });
      }
    } catch (error) {
    ...
    <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-8">
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl shadow-lg p-6 mb-6">
        <div>
          ...
          <p className="text-gray-600">Add a new Company organization to your website</p>
        </div>
      </div>
            ...
            >
              {isLoading ? <Loader /> : (
                <>
                  ...
                  <span>Add Company Associated</span>
                </>
              )}
            </button>
          </div>
